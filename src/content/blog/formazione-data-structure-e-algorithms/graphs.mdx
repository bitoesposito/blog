...
- **Edge** (arco): una connessione tra due vertici. Talvolta chiamata "connection", ma "edge" è la terminologia corretta
...
**Nota**: In questo corso implementeremo graphs con **edges bidirezionali e non pesati** per semplicità, ma è importante sapere che esistono anche graphs diretti e pesati.
...
**Nota**: In questo corso useremo l'**adjacency list** perché è più semplice da implementare e più efficiente in termini di spazio per la maggior parte dei casi d'uso.
...
### Esempi pratici:
1. **Reti sociali**: Facebook (amici), Twitter (followers), LinkedIn (connessioni)
2. **Navigazione GPS**: strade come edges, incroci come vertici
3. **Routing di rete**: router come vertici, connessioni come edges
4. **Sistemi di raccomandazione**: prodotti/utenti come vertici, preferenze come edges
5. **Dependency management**: pacchetti come vertici, dipendenze come edges