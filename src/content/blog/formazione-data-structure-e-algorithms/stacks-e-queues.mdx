...
In questa sezione esploreremo due strutture dati fondamentali: **Stacks** (pile) e **Queues** (code). Entrambe sono strutture lineari che differiscono per l'ordine di accesso agli elementi. Potremo riutilizzare molto codice dalle linked lists, rendendo questa sezione relativamente semplice.
...
**LIFO - Last In, First Out**: l'ultimo elemento aggiunto è il primo a essere rimosso.
...
**Complessità con linked list**:
- **Push** (inizio): O(1) ✅
- **Pop** (inizio): O(1) ✅
- **Push** (fine): O(1) ma pop sarebbe O(n) ❌
- **Pop** (fine): O(n) ❌ - richiede iterazione

**Importante**: Con una linked list, usa sempre **l'inizio** come cima dello stack (`head`), non la fine (`tail`), per avere entrambe le operazioni O(1).
...
**Complessità con linked list**:
- **Enqueue** (fine con `last`): O(1) ✅
- **Dequeue** (inizio con `first`): O(1) ✅
- ❌ Non usare: Enqueue dall'inizio o Dequeue dalla fine (una sarebbe O(n))
